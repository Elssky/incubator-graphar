...
int read_parquet_file_and_get_valid_intervals(
  ...
  /* for (int i = 0; i < num_columns; i++) {
    ...
    if (i < tested_label_num) {
      ...
      // !!! This is only used for debug
      // validate_column(i, row_num, repeated_nums, repeated_values, length);
    }
  } */
        ...
        for (int k = 0; k < tested_label_num; k++) {
          ...
          if (value[i].len > 0) {
            ...
            if (find_ptr != nullptr && find_ptr +
                                               label_names[tested_label_ids[k]].size() -
                                               (char*)value[i].ptr <=
                                           value[i].len) {
              state[k] = true;
            }
...
void generate_parquet_file_string_2(const char* parquet_filename, const int row_num,
  ...
  for (int rg = 0; rg < row_group_count; ++rg) {
    ...
    for (int i = 0; i < NUM_ROWS_PER_ROW_GROUP; i++) {
      ...
      int char_buffer_pos = 0;
      for (int j = 0; j < label_num; j++) {
        if (has_labels[j].find(index) != has_labels[j].end()) {
          snprintf(char_buffer + char_buffer_pos, total_length,
                   "%s:", label_names[j].c_str());
          char_buffer_pos += label_names[j].length() + 1;
        }
...
/// the label names are given by label_names, and the id column is optional.
/// The function SetupSchema() is used to setup the parquet schema.
static void generate_parquet_file_bool_2(const char* parquet_filename, const int row_num,
                                         const int label_num,
                                         std::set<int32_t> has_labels[],
                                         const std::string* label_names,
                                         const bool contain_id_column,
                                         const parquet::Encoding::type encoding_type) {
  std::cout << "Generating a parquet file: " << parquet_filename << std::endl
  ...
  for (int rg = 0; rg < row_group_count; ++rg) {
    ...
    for (int k = 0; k < label_num; k++) {
      ...
      for (int i = 0; i < NUM_ROWS_PER_ROW_GROUP; i++) {
        ...
        bool_buffer[buffer_size++] = (has_labels[k].find(index) != has_labels[k].end());
        if (buffer_size == BATCH_SIZE) {
          bool_writer->WriteBatch(buffer_size, nullptr, nullptr, bool_buffer);
          buffer_size = 0;
        }
...
void generate_parquet_file_bool_RLE_2(const char* parquet_filename, const int row_num,
                                      ...
                                      const bool contain_id_column) {
  std::cout << "Encoding type: bool(RLE)" << std::endl;
  generate_parquet_file_bool_2(parquet_filename, row_num, label_num, has_labels,
                               label_names, contain_id_column, parquet::Encoding::RLE);
}
...
int read_parquet_file_and_get_valid_intervals_2(
  ...
  for (int rg = 0; rg < row_group_count; ++rg) {
    ...
    for (int k = 0; k < tested_label_num; k++) {
      ...
      while (bool_reader->HasNext()) {
        ...
        // labels.
        // rows_read =
        //     bool_reader->ReadBatch(BATCH_SIZE, repeated_nums + length[k],
        //                            repeated_values + length[k], length[k], &values_read);
  ...
  std::cout << "The parquet file is read successfully!" << std::endl << std::endl;
  // return the valid count
  return GetValidIntervals2(tested_label_num, row_num, repeated_nums, repeated_values,
                            length, IsValid, indices, bitmap, query_type);
}
